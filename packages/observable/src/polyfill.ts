...
let ObservableCtor: ObservableCtor | undefined =
  typeof Observable === "function" ? Observable : undefined;

export function getDefaultObservableCtor<
  O extends ObservableCtor = ObservableCtor
>(): O {
  if (!ObservableCtor) {
...
export function setDefaultObservableCtor<T extends ObservableCtor>(
  Observable: T,
  overwrite = false,
) {
  if (ObservableCtor !== undefined && !overwrite) {
    throw new Error(
      "You are trying to overwrite default Observable for \
